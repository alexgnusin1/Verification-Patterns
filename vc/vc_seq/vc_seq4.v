module vc_seq4 (clk, rst_n, c1, c2, c3, c4, match);
...
reg error, match;
reg [2:0] state, next;
reg [COUNT_WIDTH-1:0] count1, count2, count3, count4;

initial
  if (C1_MAX < C1_MIN | C2_MAX < C2_MIN | C3_MAX < C3_MIN | C4_MIN == 0) begin
    $display("<%m> CONFIGURATION ERROR: Please correct parameters");
    $finish;
  end

//---------------------------------------------
always @(state or c1 or c2 or c3 or c4 or count4) begin
  next = WAIT;
  match = 0;
  error = 0;
  ...
  WAIT: if (c1)                                    next = S1;
            ...
            if (C4_MIN == 1) begin   match = 1;    next = WAIT; end
            else                                   next = S4;
  ...
  S4:   if (count4 < C4_MIN-1)
          if (c4)                                  next = S4;
          else               begin   error = 1;    next = WAIT; end
        else if (count4 == C4_MIN-1)
          if (c4)            begin   match = 1;    next = WAIT; end
          else               begin   error = 1;    next = WAIT; end
